![logo](https://raw.githubusercontent.com/FortAwesome/Font-Awesome/6.x/svgs/solid/dumbbell.svg ':size=120')

# TrAIner Hub

> Plataforma inteligente de planejamento e acompanhamento de dietas com IA

- 🏗️ Arquitetura de microsserviços moderna
- 🔐 Autenticação OAuth2 + JWT + RBAC
- 📱 App mobile React Native (iOS & Android)
- 🤖 IA para recomendações personalizadas
- 📊 10 microsserviços Kotlin + Spring Boot
- 🚀 Event-driven com RabbitMQ

[GitHub](https://github.com/MateusO97/trainer-hub-docs)
[Começar →](README.md)
